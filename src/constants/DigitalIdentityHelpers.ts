...
export const RSA = async (req, res, next) => {
  try {
...
function encryptionRSA(message : any , Key:any) {
  try {
...
export const decryptionRSA = (req, res, next) => {
  const { ciphertext, Key } = req.body;
  const message = decryptionCiphertext(ciphertext, Key);
  res.send({ message: message.toString() });
};

export const customKeyToForgeKey = (publicKey : any , privateKey:any) => {
...
export const digitalSigning = async (req, res, next) => {
  try {
    const { message, privateKey } = req.body;

    const hash = sha256(message);
    const privateKey2 = CryptoJS.createPrivateKey({
...
export const verifySignature = (req, res, next) => {
  try {
    ...
    const privateKey2 = CryptoJS.createPublicKey({
...
function sha256(input:any) {
  return CryptoJS.createHash("sha256").update(input).digest("hex");
}
