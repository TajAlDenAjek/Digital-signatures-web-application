...
const GovernmentmangeDigitalCertifcates = () => {
    const {
        data,
        currentData,
        isLoading,
    ...
    if (isLoading) {
        ...
        content = <AdminTable customActions={customActions} columns={digitalCertificatesOrdersColumns} data={currentData.filter((item)=>item?.reqStatus!='pending')} tableTitle={"Digital Certificate Request"} ModalContent={CustomComponent} actions={['view']} />
    } else if (isError) {
        content = <>{error}</>
    }
    return (
...
export default GovernmentmangeDigitalCertifcates